"""Build the seven-reader source-only SyncG overview."""
...
def main():
    with (ROOT / "data/figure_inputs_shared_full_20260911/seed_mean_sample_sd.csv").open(encoding="utf-8-sig") as stream:
        rows = {row["method"]: row for row in csv.DictReader(stream)
                if row["family"] == "main" and row["dataset"] == "syncg" and row["scope"] == "all_conditions"}
    assert set(rows) == set(METHODS)
    assert all(int(rows[method]["seeds"]) == 3 for method in METHODS)
    plt.rcParams.update({
    ...
    for axis, metric, label, title in zip(axes, ["nmae_pct_fs", "acc_at_5_pct"],
                                        ["NMAE (%FS)", "Acc@5%"], ["(a) SyncG NMAE", "(b) SyncG Acc@5%"]):
        values = np.array([float(rows[method][metric + "_mean"]) for method in METHODS])
        errors = np.array([float(rows[method][metric + "_sample_sd"]) for method in METHODS])
        ...
        for i, (value, error) in enumerate(zip(values, errors)):
            axis.text(value + error + .025 * upper, i, f"{value:.1f}" if metric == "acc_at_5_pct" else f"{value:.2f}",
                      va="center", fontsize=8.5, fontweight="bold" if METHODS[i] == "geoprr" else "normal")
    stem = HERE / "fig_syncg_overview"
